...
class TestPrincComps(unittest.TestCase):
    ...
    def test_fit_transform(self):
        print("Preprocessing: Testing PCA fit/transform")
        princ_comp = get("principal_components")()
        princ_comp.fit(X=self.predictor_data)
        new_data = np.array([[0.5355, 22.42513, 0.5225, 2.34411]])
        transformed_data = princ_comp.transform(X=new_data)
        self.assertEqual(transformed_data[0][0], np.float64(0.3135231903646593))

    def test_graph_gen(self):
        print("Preprocessing: Testing PCA ONNX generation")
        princ_comp = get("principal_components")(num_components=1)
        princ_comp.fit(X=self.predictor_data)
        graph = princ_comp.create_onnx_graph()
        ...
        self.assertEqual(val[0][0], np.float32(0.17238933))
