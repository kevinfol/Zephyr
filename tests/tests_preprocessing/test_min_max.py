...
class TestMinMax(unittest.TestCase):
    ...
    def test_fit_transform(self):
        print("Preprocessing: Testing MinMax fit/transform")
        std = get("minmax")()
        std.fit(X=self.predictor_data)
        new_data = np.array([[0.5355, 22.42513, 0.5225, 2.34411]])
        transformed_data = std.transform(X=new_data)
        self.assertEqual(transformed_data[0][0], np.float64(0.4886295987617573))

    def test_graph_gen(self):
        print("Preprocessing: Testing MinMax ONNX generation")
        std = get("minmax")()
        std.fit(X=self.predictor_data)
        ...
        self.assertEqual(val[0][0], np.float32(0.48862955))
