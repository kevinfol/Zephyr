...
class TestNoOp(unittest.TestCase):
    ...
    def test_fit_transform(self):
        print("Preprocessing: Testing NO-OP fit/transform")
        noop = get("none")()
        noop.fit(X=self.predictor_data)
        new_data = np.array([[0.5355, 22.42513, 0.5225, 2.34411]])
        transformed_data = noop.transform(X=new_data)
        self.assertEqual(transformed_data[0][0], np.float64(0.5355))

    def test_graph_gen(self):
        print("Preprocessing: Testing NO-OP ONNX generation")
        noop = get("none")()
        noop.fit(X=self.predictor_data)
        graph = noop.create_onnx_graph()
        ...
        self.assertEqual(val[0][0], np.float32(0.5355))
