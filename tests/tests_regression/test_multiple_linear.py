...
class TestMLR(unittest.TestCase):
    ...
    def test_fit_predict(self):
        print("Regression: Testing MLR")
        mlr = MultipleLinearRegression(False)
        mlr.fit(self.predictor_data, self.target_data)
        self.assertAlmostEqual(mlr.coef_[0], 2.0, 2)
        self.assertAlmostEqual(mlr.coef_[1], 4.0, 2)
        self.assertAlmostEqual(mlr.coef_[2], 3.0, 2)
        self.assertAlmostEqual(mlr.coef_[3], 1.0, 2)
        self.assertAlmostEqual(mlr.intercept_, 6.7769992, 2)
        predictions = mlr.predict(self.predictor_data)
        self.assertAlmostEqual(predictions[0], 118.3346, 2)

    def test_graph_gen(self):
        print("Regression: Testing MLR ONNX")
        mlr = MultipleLinearRegression(False)
        mlr.fit(self.predictor_data, self.target_data)
        graph = mlr.create_onnx_graph()
        ...
        self.assertAlmostEqual(val[0], np.float32(118.3346), 2)
